...
Now we have all necessary tools to transform the master equation~\eqnref{master-eqn:master-eqn} with the Wigner transformation ~\eqnref{func-wigner:w-transformation} \todo{need a specific definition for multi-component transformation?} to the partial differential equation.
...
These equations can now be solved using conventional methods, and any required expectations symmetrically ordered operator products can be obtained from their solution using \thmref{func-wigner:mc-moments}:
\begin{eqn}
    \langle \symprod{
